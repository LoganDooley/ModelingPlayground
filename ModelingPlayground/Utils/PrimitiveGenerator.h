...
{
public:
    static std::pair<std::vector<float>, std::vector<unsigned int>> GenerateSphere(
        int latitudinalResolution, int longitudinalResolution);
    static std::pair<std::vector<float>, std::vector<unsigned int>> GenerateCube();
...
private:
    ...
    static void GenerateCubeFace(std::vector<float>& vertices, std::vector<unsigned int>& indices,
                                 glm::vec3 faceNormal);
