...
struct adl_serializer<std::shared_ptr<Component>>
{
    static inline std::unordered_map<std::string, ComponentSerializer_Impl::Serializer> m_serializers = {
        {typeid(DirectionalLightComponent).name(), ComponentSerializer_Impl::SerializerFor<DirectionalLightComponent>()},
        {typeid(MaterialComponent).name(), ComponentSerializer_Impl::SerializerFor<MaterialComponent>()},
        {typeid(OpenGLSettingsComponent).name(), ComponentSerializer_Impl::SerializerFor<OpenGLSettingsComponent>()},
    ...
    static void to_json(nlohmann::json& json, const std::shared_ptr<Component>& component)
    {
        if (component)
        {
            ...
            {
                std::cout << "adl_serializer<std::shared_ptr<Component>>|to_json: " << typeName << " not registered!\n";
                return;
            }
    ...
    static void from_json(const nlohmann::json& json, std::shared_ptr<Component>& component)
    {
        if (json.is_null())
        {
            component = nullptr;
        ...
        else
        {
            ...
            std::string typeName = typeJson.get<std::string>();
