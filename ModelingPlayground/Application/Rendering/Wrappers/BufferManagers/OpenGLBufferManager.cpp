...
void OpenGLBufferManager::RegenerateBuffers()
{
    ...
    m_sceneHierarchy->BreadthFirstProcessAllSceneNodes(
        [this, &drawableCount, &lightCount](std::shared_ptr<SceneNode> sceneNode)
        {
            ...
            if (sceneNode->ContainsComponent<PointLightComponent>() || sceneNode->ContainsComponent<
                SpotLightComponent>() || sceneNode->ContainsComponent<DirectionalLightComponent>())
            {
                lightCount++;
            }
        });
