...
std::pair<glm::vec3, glm::vec3> SceneViewCamera::GetWorldSpaceRayThroughPixel(int x, int y) const
{
    float normalizedX = 2 * (static_cast<float>(x) / static_cast<float>(m_screenSize.x) - 0.5f);
    float normalizedY = -2 * (static_cast<float>(y) / static_cast<float>(m_screenSize.y) - 0.5f);

    auto worldSpacePixelLocation = glm::vec3(normalizedX, normalizedY, 1.0f);
    worldSpacePixelLocation = glm::inverse(m_projectionMatrix) * glm::vec4(worldSpacePixelLocation, 1.0f);
    worldSpacePixelLocation = glm::inverse(m_viewMatrix) * glm::vec4(worldSpacePixelLocation, 1.0f);
    return {m_position, glm::normalize(worldSpacePixelLocation - m_position)};
}
