    ...
    m_inputManager(inputManager)
{
    ...
    m_inputManager->SubscribeToMouseButtonEvents([this](int button, int action)
    {
        if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_LEFT)
        {
            glm::vec2 sceneViewCursorPos = m_inputManager->GetSceneViewCursorPos();
            if (sceneViewCursorPos.x >= 0 && sceneViewCursorPos.y >= 0 && sceneViewCursorPos.x < m_camera->
                GetScreenSize().x && sceneViewCursorPos.y < m_camera->GetScreenSize().y)
            {
                m_renderingManager->SelectObjectAtPixel(sceneViewCursorPos.x, sceneViewCursorPos.y);
            }
        }
    });
}
