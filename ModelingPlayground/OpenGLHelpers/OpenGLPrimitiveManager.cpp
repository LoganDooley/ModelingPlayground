...
void OpenGLPrimitiveManager::GeneratePrimitives(int sphereLatitudinalResolution, int sphereLongitudinalResolution)
{
	...
	std::pair<std::vector<OpenGLVertex>, std::vector<int>> sphereData = PrimitiveGenerator::GenerateSphere(
...
std::string OpenGLPrimitiveManager::LoadPrimitive(std::string filePath)
{
	...
	{
		std::cout <<
			"OpenGLPrimitiveManager|LoadPrimitive: More than 2 materials per .obj specified. Currently no support for per-face materials\n";
		return "";
	}
	...
	for (size_t shapeIndex = 0; shapeIndex < shapes.size(); shapeIndex++)
	{
		...
		for (size_t faceIndex = 0; faceIndex < shapes[shapeIndex].mesh.num_face_vertices.size(); faceIndex++)
		{
			...
			for (size_t v = 0; v < faceVertexCount; v++)
			{
				...
				{
					std::cout <<
						"OpenGLPrimitiveManager|LoadPrimitive: No normals present, currently require normals\n";
					return "";
				}
